...
class TWW_Glossary_Scraper {
    ...
    public function register_rest_routes() {
        ...
        register_rest_route('tww-glossary/v1', '/convert-hash-tags', [
            'methods' => 'POST',
            'callback' => [$this, 'convert_and_store_studies'],
            'permission_callback' => '__return_true',
        ]);
    }

    // This is an example of a block of html that consists of a study. We need to convert the hashtags into post_tag links, remove the hashtags, and do not include the MedScienceResearch hashtag.
    // there are many of these blocks on the page that need to be scraped and stored, as posts, however we first need to check if the study exists before creating a new post.
    //
    ...
    // ])
    // ->set_max(-1)
    // tww_study_single_description
    // tww_study_single_links
    //
    ...
    //      <div class="study">
    //          <h4>
    //              <span class="title-text">A randomized, placebo-controlled trial of topical retinol in the treatment of cellulite</span>
    //          </h4>
    //          <div>Cellulite occurs to varying degrees on the thighs and buttocks of many otherwise healthy women. Among the many purported treatments for cellulite, only a handful have been tested in clinical trials. The aim of this study was to critically explore the reputed effect of topical retinol in the treatment of cellulite. After 6-months of treatment, skin elasticity was increased by 10.7% while viscosity was decreased by 15.8% at the retinol-treated site. Such an effect on the tensile properties of skin was more prominent where the mattress phenomenon was the only evidence of cellulite. The lumpy-bumpy appearance of the skin showed either little response or was not responsive to the treatment. Although gross microanatomical differences were not disclosed between the comparative sites at completion of the study, evidence for a shift in the phenotype of connective tissue cells was obtained. The main retinol-related change consisted of a 2- to 5-fold increase in the number of factor XIIIa+ dendrocytes both in the dermis and fibrous strands of the hypodermis.</div>
    //          <div class="hashtags">#Rabies #Paralysis #Neurological #Encephalitis #Vaccine #MedScienceResearch </div>
    //          <br>
    //         <ul style="margin: 0; padding: 0;">
    //                 <li style="list-style: none;"><a target="_blank" href="https://pubmed.ncbi.nlm.nih.gov/11702613/">https://pubmed.ncbi.nlm.nih.gov/11702613/ <i class="fa-solid fa-up-right-from-square"></i></a></li>
    //         </ul>
    //      </div>

    public function convert_and_store_studies(\WP_REST_Request $request) {
        ...
        if ($post_id) {
            ...
            // Get all the study div elements
            $studyDivs = $xpath->query('//div[contains(@class, "study")]');
    
            foreach ($studyDivs as $studyDiv) {
                // Extract the title
                $titleNode = $xpath->query('.//h4', $studyDiv)->item(0);
                $title = $titleNode ? $titleNode->textContent : '';

                // Extract the description
                $descriptionNode = $xpath->query('.//div[@class="description"]', $studyDiv)->item(0);
                $description = $descriptionNode ? $descriptionNode->textContent : '';

                // Extract hashtags and convert them to post_tag links, excluding #MedScienceResearch
                $hashtagsNode = $xpath->query('.//div[@class="hashtags"]', $studyDiv)->item(0);
                $hashtags = $hashtagsNode ? $hashtagsNode->textContent : '';
                $hashtags = preg_replace('/#MedScienceResearch\s*/', '', $hashtags); // Remove #MedScienceResearch
                $hashtags = preg_replace('/#(.*?)(\s|$)/', '<a class="study-tag" href="#">$1</a>', $hashtags);
    
                // Extract the link
                $linkNode = $xpath->query('.//a', $studyDiv)->item(0);
                $link = $linkNode ? $linkNode->getAttribute('href') : '';
    
                // Create a new post for each study
                // the study content goes in tww_study_single_description not post_content
                $new_post = array(
                    'post_title'   => $title,
                    'post_status'  => 'publish',
                    'post_type'    => 'tww_study_single',
                    'meta_input'   => array(
                        '_tww_study_single_links' => array(
                            array(
                                'link_url' => $link,
                                'link_text' => $link,
                            ),
                        ),
                        '_tww_study_single_description' => $description,
                    ),
                );
                ...
                if ($new_post_id && !is_wp_error($new_post_id)) {
                    carbon_set_post_meta($new_post_id, 'tww_study_single_description', $description);
                
                    // For complex fields like links, use an array structure
                    $links = array(
                        array(
                            'link_url' => $link,
                            'link_text' => $link,
                    ...
                    carbon_set_post_meta($new_post_id, 'tww_study_single_category[0]', [$array]);

                    carbon_set_post_meta($new_post_id, 'tww_study_single_links', $links);
                }
    
                // Assign tags to the new post
                $tags = [];
                preg_match_all('/>(.*?)<\/a>/', $hashtags, $tag_matches);
